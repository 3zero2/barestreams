...
const fetchAllTorrents = async (
	...
	let expectedTotal =
		typeof firstResponse.torrents_count === "number"
			? firstResponse.torrents_count
			: null;
	let pageLimit =
		typeof firstResponse.limit === "number" && firstResponse.limit > 0
			? firstResponse.limit
			: DEFAULT_LIMIT;
	...
	const totalPages = expectedTotal
		? Math.ceil(expectedTotal / pageLimit)
		: MAX_PAGES;
...
const matchesTitleText = (text: string, titles: string[]): boolean => {
	...
	return titles.some((title) => {
		...
		const overlap = calcTokenOverlap(
			tokenizeTitle(normalizedText),
			needleTokens,
		);
		return overlap >= 0.6;
	});
...
const extractSizeLabel = (html: string): string | null => {
	const match = html.match(
		/(\d+(?:\.\d+)?\s*(?:TB|GB|MB|KB|TiB|GiB|MiB|KiB))/i,
	);
...
export const scrapeEztvStreams = async (
	...
	const streams = torrents
		...
		.filter((torrent) =>
			matchesEpisode(torrent, parsed.season, parsed.episode),
		)
		.map((torrent) => {
			...
			const cleanedTorrentName = rawTorrentName
				.replace(/[.\s-]*\bEZTV\b\s*$/i, "")
				.trim();
			const torrentName = cleanedTorrentName || rawTorrentName;
			const imdbTitle =
				basics?.primaryTitle || basics?.originalTitle || "EZTV";
